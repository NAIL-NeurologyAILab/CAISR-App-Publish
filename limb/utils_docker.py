import sys, os, h5py, warnings, mne
import numpy as np
import pandas as pd
...
#     # eeg = ['c3-m2', 'c4-m1']
#     # import pdb; pdb.set_trace()
#     select_channels = common_sigs
#     EMG = data[select_channels]
    
#     return EMG, original_Fs

def select_signals_cohort(path):
    """
    Loads limb EMG signals from a file with robust channel selection.
    
    This function can handle three scenarios for each leg:
    1. A single pre-processed channel (e.g., 'lat' or 'rat').
    2. A differential pair (e.g., 'lleg+' and 'lleg-'), which it will subtract.
    3. The channel is missing.

    Args:
        path (str): Path to the data file.

    Returns:
        pd.DataFrame: A DataFrame containing the final EMG signals, with one or two columns.
        int: The original sampling frequency.
    """
    original_Fs = 200
    data, params = load_prepared_data(path)
    data = data.astype(np.float64)
    ...
    # This will hold our final processed EMG signals
    emg_signals = pd.DataFrame()

    # --- Process Left Leg ---
    if 'lat' in available_channels:
        print("Found channel 'lat'.")
        emg_signals['left_leg'] = data['lat']
    elif 'plml' in available_channels:
        print("Found channel 'plml'.")
        emg_signals['left_leg'] = data['plml']
    elif all(ch in available_channels for ch in ['lleg+', 'lleg-']):
        print("Found differential pair 'lleg+/-'. Calculating signal.")
        emg_signals['left_leg'] = data['lleg+'] - data['lleg-']
    else:
        print("Warning: No left leg EMG channel found.")

    # --- Process Right Leg ---
    if 'rat' in available_channels:
        print("Found channel 'rat'.")
        emg_signals['right_leg'] = data['rat']
    elif 'plmr' in available_channels:
        print("Found channel 'plmr'.")
        emg_signals['right_leg'] = data['plmr']
    elif all(ch in available_channels for ch in ['rleg+', 'rleg-']):
        print("Found differential pair 'rleg+/-'. Calculating signal.")
        emg_signals['right_leg'] = data['rleg+'] - data['rleg-']
    else:
        print("Warning: No right leg EMG channel found.")

    # --- Final Check ---
    if emg_signals.empty:
        # If the DataFrame is still empty, we found no usable channels
        raise ValueError(f"No recognizable limb EMG channels found in file: {path}")

    # The original script processes only the FIRST channel it finds.
    # To maintain that behavior, we'll return the signals in a consistent order:
    # left leg first, if it exists.
    final_order = []
    if 'left_leg' in emg_signals.columns:
        final_order.append('left_leg')
    if 'right_leg' in emg_signals.columns:
        final_order.append('right_leg')
        
    final_emg = emg_signals[final_order]

    # The rest of the script expects a NumPy array, so we return .values
    return final_emg.values, original_Fs
