...
def select_signals_cohort(path):
    ...
    # If any channels are missing, print a warning
    if channels_missing:
        the_id = path.split('/')[-1].split('.')[0]
        print(f'   [WARNING] File "{the_id}" is missing channels: {channels_missing}. They will be replaced with zeros.')

    # Select only the available signals from the data
    signals = data[channels_found]
    
    # Add placeholder columns of zeros for any missing signals
    for missing_ch in channels_missing:
        signals[missing_ch] = 0
